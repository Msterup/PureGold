...
checkpoint = Path('checkpoints/2021-09-20T01-43-09/mario_net_24.chkpt')
    ...
    while True:
        ...
        if all_same == True:
            ...
            for each in range(1,piles+1):
                loss, prediction = agent.act(board.make_move(each-1), score[each])
                loss_sum += loss
            ...
            print(predictions)
            print(" ")
            print(f"Neuraln option was {pred_card}, loss_sum was {loss_sum}")
            ...
            writer.add_scalar("Prediction mean of last 1000", torch.FloatTensor([st.mean(prediction_list_moving)]),
                              cards_trained)
            writer.add_scalar("loss_sum", torch.FloatTensor([loss_sum]), cards_trained)
